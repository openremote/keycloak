...
function inDesignOrder(attributes: Attribute[]): Attribute[] {
  const known = DESIGN_ORDER.map(name => attributes.find(a => a.name === name)).filter(
    (a): a is Attribute => a !== undefined
  );
...
/**
 * Every attribute of the realm's user profile, in the design's order.
 *
 * Shared by the three pages Keycloak drives from the user profile - registration, the
 * update-profile required action, and the identity-provider profile review - because they
 * render the same form; they differ only in the chrome around it.
 */
export function profileFields(kcContext: KcContext, i18n: I18n): TemplateResult {
  const profile = (kcContext as { profile?: { attributesByName: Record<string, Attribute> } })
    .profile;
  const { realm, locale } = kcContext;

  /*
   * No user profile in the model means the server is older than this theme supports.
   *
   * Keycloak only drives these pages from the declarative user profile once it is always on, in 26;
   * 22-to-25 shipped it behind a feature flag and, with the flag off, sends a fixed field list as
   * `register.formData` and no `profile` at all. Rendering nothing is the only honest thing this
   * can do - there is no metadata to render from - but doing it silently is how a registration form
   * with nothing but the two password fields reached a test server. So it says why, once, where
   * whoever deployed it will see it. Keycloakify's `keycloakVersionTargets` in package.json says
   * the same thing: this jar is not built for 22-to-25.
   */
  if (profile === undefined) {
    console.error(
      `[openremote theme] ${kcContext.pageId} got no user profile from Keycloak, so its fields ` +
        "cannot be rendered. This theme needs Keycloak 26 or newer, where the declarative user " +
        "profile is always on; 22-to-25 with the feature off sends register.formData instead."
    );
    return html``;
  }

  /*
   * With internationalization on, Keycloak adds a `locale` attribute to the profile. It is not
   * a question for the user: its job is to carry the language they are reading the page in onto
   * the account. Keycloak's own user-profile-commons.ftl special-cases it exactly this way, and
   * without it the field renders as a text input labeled "locale" and posts back empty.
   */
  const isCarriedLocale = (name: string): boolean =>
    name === "locale" && !!realm.internationalizationEnabled && !!locale?.currentLanguageTag;

  return html`${inDesignOrder(Object.values(profile.attributesByName)).map((attribute, index) =>
...
function input(
  ...
  // Every html5 input type without a component of its own - tel, url, number, date - is a text field.
  // hasOwn, not `in`: the type comes from realm configuration, and `in` also matches "constructor".
  const tag = TAGS[Object.hasOwn(TAGS, type) ? (type as keyof typeof TAGS) : "text"];
