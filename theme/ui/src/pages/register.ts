...
import { errorOf, field, layout, resolvedHeading, submitButton } from "../layout";
...
export function render(kcContext: PageContext, i18n: I18n): TemplateResult {
  ...
  return layout({
    kcContext,
    i18n,
    /* A key, resolved - and if the bundle does not have it, the page's own title rather than the
       key itself. See the longer note in info.ts, where that gap actually bites. */
    heading: resolvedHeading(messageHeader, advancedMsgStr) ?? msgStr("registerTitle"),
